...
const COLOR_MAP = {
  entity:     { bg: '#8b5cf6', border: '#a78bfa', glow: 'rgba(139,92,246,0.4)'  },
  preference: { bg: '#ec4899', border: '#f472b6', glow: 'rgba(236,72,153,0.4)'  },
  goal:       { bg: '#14b8a6', border: '#2dd4bf', glow: 'rgba(20,184,166,0.4)'  },
  event:      { bg: '#f97316', border: '#fb923c', glow: 'rgba(249,115,22,0.4)'  },
  fact:       { bg: '#10b981', border: '#34d399', glow: 'rgba(16,185,129,0.4)'  },
  document:   { bg: '#0ea5e9', border: '#38bdf8', glow: 'rgba(14,165,233,0.4)'  },
  category:   { bg: '#f59e0b', border: '#fcd34d', glow: 'rgba(245,158,11,0.5)'  },
};
...
function MindmapNode({ id, data, selected }) {
  ...
  return (
    <div
      className={`mindmap-node ${selected ? 'is-active' : ''} ${isHighlighted ? 'is-cited' : ''}`}
      style={{ '--node-color': colors.bg, '--node-glow': colors.glow }}
    >
      <div className="px-4 pt-3.5 pb-3">
        ...
        <div className="flex items-center gap-1.5 mb-1.5">
          ...
          />
          <span
            className="text-[9px] uppercase tracking-[0.18em] font-bold truncate"
...
function CategoryNode({ id, data, selected }) {
  ...
  return (
    <div
      className={`category-node ${selected || isHighlighted ? 'is-active' : ''}`}
      style={{
    ...
    >
      ...
      <div className="flex flex-col items-center gap-1">
        <div
          ...
          style={{ backgroundColor: colors.bg, boxShadow: `0 0 10px ${colors.glow}` }}
        />
        <span
...
function FilterToggles({ visibleTypes, onToggle }) {
  return (
    <div className="hidden sm:flex items-center gap-1 px-2 py-1.5 rounded-2xl glass-panel flex-shrink-0 max-w-full overflow-x-auto">
      {FILTER_GROUPS.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg transition-all duration-200 cursor-pointer border ${
              isOn ? '' : 'opacity-40 hover:opacity-75 border-transparent'
            }`}
            style={
              isOn
                ? {
                    backgroundColor: `${colors.bg}1c`,
                    borderColor: `${colors.bg}50`,
                    boxShadow: `0 0 10px ${colors.bg}20`,
                  }
                : undefined
            }
          ...
          >
            <div
              ...
              style={{
                ...
                boxShadow: isOn ? `0 0 7px ${colors.glow}` : 'none',
              }}
            />
            <span
              className="text-[10px] font-semibold transition-colors"
              style={{ color: isOn ? colors.border : undefined }}
            >
              {item.label}
            </span>
            {isOn ? (
...
function GraphCanvasInner() {
  ...
  const initialNodes = useMemo(() => {
    ...
    return (graphData?.nodes || [])
      .filter((n) => (seen.has(n.id) ? false : (seen.add(n.id), true))) // dedupe ids
      .map((n) => ({
        ...n,
        type: n.type || 'entity',
  ...
  const initialEdges = useMemo(() => {
    ...
    return (graphData?.edges || [])
      .filter((e) => (seen.has(e.id) ? false : (seen.add(e.id), true))) // dedupe ids
      .map((e) => ({
        ...e,
        // Cohesive luminous-indigo edges (overrides the harsh default grey)
        style: { ...(e.style || {}), stroke: 'rgba(129,140,248,0.5)', strokeWidth: 1.6 },
        markerEnd: { type: MarkerType.ArrowClosed, color: '#818cf8' },
        labelStyle: { fill: '#a5b4fc', fontSize: 10, fontWeight: 600 },
        labelBgStyle: { fill: '#0a0d1d', fillOpacity: 0.92 },
        labelBgPadding: [6, 3],
  ...
  /* Re-fit the viewport whenever the graph grows/shrinks (e.g. after ingest) */
  useEffect(() => {
    if (!initialNodes.length) return;
    const t = setTimeout(() => fitView({ padding: 0.25, duration: 500 }), 150);
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-subtle">
      ...
      <header className="flex flex-wrap items-start gap-3 px-4 sm:px-6 py-3 border-b border-white/[0.06] glass flex-shrink-0 z-10">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <div className="w-9 h-9 rounded-2xl bg-gradient-to-br from-secondary/30 via-primary/20 to-transparent flex items-center justify-center ring-1 ring-secondary/25 flex-shrink-0 shadow-[0_0_18px_rgba(34,211,238,0.2)]">
            <Network className="w-4 h-4 text-secondary-light" />
          </div>
          <div className="min-w-0">
            <h2 className="font-display text-sm font-bold text-text-primary tracking-tight">Mind Map</h2>
      ...
      <div className="flex-1 relative">
        ...
        >
          <Background color="#28305c" gap={30} size={1.2} />
        ...
        {isEmpty && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center pointer-events-none px-6">
            <div className="relative w-20 h-20 mb-5">
              <div className="absolute inset-[-14px] rounded-full bg-gradient-to-br from-secondary/20 via-primary/15 to-transparent blur-2xl animate-breathe" />
              <div className="orb-ring" style={{ animationDuration: '10s' }} />
              <div className="absolute inset-[4px] rounded-full bg-[#0a0d1d]/85 flex items-center justify-center">
                <Network className="w-8 h-8 text-secondary-light/80" strokeWidth={1.5} />
              </div>
            </div>
            <h3 className="font-display text-lg font-bold hero-gradient-text mb-2">
              Your graph is waiting
            </h3>
            <p className="text-xs text-text-muted max-w-xs leading-relaxed">
              Chat or ingest a document and watch your knowledge crystallise into a living
              mind map — entities, goals, preferences and facts, all connected.
            </p>
          </div>
        )}
        ...
        {isRetrievalMode && neighborIds && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2.5 px-4 py-2 rounded-2xl glass-strong border !border-success/40 shadow-[0_0_24px_rgba(52,211,153,0.2)] text-[11px] animate-fade-in">
        ...
        {!isRetrievalMode && focusedNodeId && neighborIds && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2.5 px-4 py-2 rounded-2xl glass-strong border !border-primary/40 shadow-[0_0_24px_rgba(99,102,241,0.2)] text-[11px] animate-fade-in">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary-light opacity-60" />
        ...
        {/* Hint overlay */}
        {!focusedNodeId && !selectedNodeData && !isEmpty && (
          <div className="absolute top-4 left-4 z-10 flex items-center gap-2 px-3 py-2 rounded-xl glass text-[11px] text-text-muted/60 border border-white/[0.05] animate-fade-in pointer-events-none">
        ...
        {/* ── Node Detail Panel ── */}
        {selectedNodeData && (
          <div className="absolute top-20 right-4 w-72 sm:w-80 glass-strong rounded-3xl overflow-hidden animate-fade-in-scale z-[50] max-h-[calc(100vh-120px)] flex flex-col">
            <div
              className="h-1 flex-shrink-0"
              style={{
                background: `linear-gradient(90deg, ${getNodeColor(selectedNodeData.data.nodeType)?.bg || '#8b5cf6'}, transparent)`,
              }}
            />
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06] flex-shrink-0">
              <div className="flex items-center gap-2.5">
                <Info className="w-4 h-4 text-primary-light" />
                <span className="font-display text-sm font-bold text-text-primary">Node Details</span>
              </div>
              <button
            ...
            <div className="p-5 space-y-5 overflow-y-auto">
              <div className="flex items-center gap-2.5">
                <div
                  ...
                  style={{
                    backgroundColor: getNodeColor(selectedNodeData.data.nodeType)?.bg || '#8b5cf6',
                    boxShadow: `0 0 10px ${getNodeColor(selectedNodeData.data.nodeType)?.glow || 'transparent'}`,
                  }}
                />
                <span
                ...
                <span className="text-[10px] font-mono text-text-muted ml-auto px-1.5 py-0.5 rounded-md bg-white/[0.04] border border-white/[0.06]">
                  {selectedNodeData.id}
                </span>
              </div>
              <DetailField
